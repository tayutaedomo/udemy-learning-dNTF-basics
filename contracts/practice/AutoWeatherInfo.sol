...
contract AutoWeatherInfo is ChainlinkClient {
    /// @dev Chainlink の Struct の Request 型にアタッチ
    using Chainlink for Chainlink.Request;

    /* ========== CONSUMER STATE VARIABLES ========== */
    // @dev リクエストパラメータを記録するための構造体
    ...
    function requestCurrentConditions(
        ...
        // リクエストを実施し、requestId を得る
        requestId = sendChainlinkRequest(req, _payment);

        // requestId に紐づけて結果を記録する
    ...
    /* ========== OTHER FUNCTIONS ========== */
    /// @dev 登録した OracleContract アドレスを戻す
    function getOracleAddress() external view returns (address) {
        return chainlinkOracleAddress();
    }

    /// @dev OracleContract アドレスを設定する
    function setOracle(address _oracle) external {
        setChainlinkOracle(_oracle);
    }

    /// @dev 預けている LINK トークンを引き出す
